...
var cssModuleToTypescriptInterfaceProperties = function cssModuleToTypescriptInterfaceProperties(cssModuleKeys) {
  ...
  var useSemicolons = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;

  return cssModuleKeys.map(function (key) {
    return indent + '\'' + key + '\': string' + (useSemicolons ? ';' : '');
  }).join('\n');
};

var cssModuleToNamedExports = function cssModuleToNamedExports(cssModuleKeys, useSemicolons) {
  return cssModuleKeys.map(function (key) {
    return 'export const ' + key + ': string' + (useSemicolons ? ';' : '');
  }).join('\n');
};
...
var generateNamedExports = exports.generateNamedExports = function generateNamedExports(cssModuleKeys) {
  var useSemicolons = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;

  var namedExports = cssModuleToNamedExports(cssModuleKeys, useSemicolons);
  return namedExports + '\n';
};
...
var generateGenericExportInterface = exports.generateGenericExportInterface = function generateGenericExportInterface(cssModuleKeys, filename, indent) {
  var useSemicolons = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;

  var interfaceName = filenameToInterfaceName(filename);
  var interfaceProperties = cssModuleToTypescriptInterfaceProperties(cssModuleKeys, indent, useSemicolons);
